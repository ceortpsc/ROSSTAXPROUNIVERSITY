...
export async function registerEnrollmentRoutes(app: FastifyInstance) {
  app.get('/api/enrollment/readiness', async () => ({
    ok: true,
    system: 'Ross Tax Pro University Enrollment Conversion System',
    ...enrollmentReadiness(),
  ...
  app.post('/api/enrollment/invites', async (request, reply) => {
    ...
    try {
      ...
      const delivery = await deliverInvite(inviteUrl, generated.invite).catch((error) => ({
        attempted: true,
        delivered: false,
        provider: 'resend',
      ...
      return reply.code(201).send({
        ok: true,
        invite: generated.invite,
        inviteUrl,
        delivery,
        evidence: {
          inviteTokenSha256Prefix: createHash('sha256').update(generated.token).digest('hex').slice(0, 24),
          recipientEmailSha256Prefix: createHash('sha256').update(generated.invite.recipientEmail).digest('hex').slice(0, 24)
        }
      });
    } catch (error) {
    ...
    try {
      ...
      return {
        ...
        invite: {
          ...
          programTitle: invite.programTitle,
  ...
  app.post('/api/enrollment/applications', async (request, reply) => {
    try {
      ...
      request.log.info({
        ...
        inviteId: application.inviteId,
        audience: application.audience,
        programId: application.program.id,
        delivered: dispatch.delivered,
        channels: dispatch.attempts.map((item) => item.channel)
      ...
      if (!dispatch.delivered) {
        return reply.code(503).send({
          ok: false,
          submitted: false,
          error: 'The application was signed and converted, but RTPU delivery is not configured or did not succeed. Download the export package and contact the enrollment office; do not assume submission was completed.',
          applicationId: application.applicationId,
