...
app.get('/api/integrations/google-classroom/oauth/callback', async (request, reply) => {
  ...
  if (query.error) {
    return reply.code(400).type('text/html').send(consentPage('Google Classroom authorization declined', `<p class="warn">Authorization was not completed</p><h1>Google returned an OAuth error</h1><p>${htmlEscape(query.error)}</p>`));
  }
  if (!query.code || !verifyGoogleClassroomOAuthState(query.state || null)) {
    return reply.code(400).type('text/html').send(consentPage('Invalid OAuth callback', '<p class="warn">OAuth state validation failed</p><h1>Start a fresh authorization</h1><p>The authorization must be started from RTPU and completed within 10 minutes.</p>'));
  }

  const clientId = process.env.GOOGLE_CLASSROOM_CLIENT_ID;
  const clientSecret = process.env.GOOGLE_CLASSROOM_CLIENT_SECRET;
  if (!clientId || !clientSecret) {
    return reply.code(503).type('text/html').send(consentPage('Connector not configured', '<h1>Google OAuth client credentials are missing</h1>'));
  }
  ...
  if (!tokenResponse.ok) {
    return reply.code(502).type('text/html').send(consentPage('OAuth exchange failed', `<h1>Google token exchange failed</h1><p>${htmlEscape(token.error_description || token.error || 'Unknown OAuth error')}</p>`));
  }
  if (!token.refresh_token || !token.access_token) {
    return reply.code(409).type('text/html').send(consentPage('Offline authorization incomplete', '<h1>Google did not issue the required offline credentials</h1><p>Restart authorization with consent so Google can issue an access token and refresh token.</p>'));
  }

  const authHeaders = { Authorization: `Bearer ${token.access_token}` };
  const [profileResponse, coursesResponse] = await Promise.all([
    fetch('https://classroom.googleapis.com/v1/userProfiles/me', { headers: authHeaders }),
    fetch('https://classroom.googleapis.com/v1/courses?pageSize=1', { headers: authHeaders })
  ]);
  ...
  const receipt = {
    ...
    googleClassroom: {
      ...
      emailDomain: profile.emailAddress?.split('@')[1]?.toLowerCase() || null
    },
    endToEndAuthorizationVerified: profileResponse.ok && coursesResponse.ok
  };

  if (!receipt.endToEndAuthorizationVerified) {
    request.log.error({ receipt }, 'Google Classroom consent verification failed');
    return reply.code(502).type('text/html').send(consentPage('Google Classroom verification failed', `<p class="warn">Token exchange succeeded, but Classroom API verification failed</p><div class="code">${htmlEscape(JSON.stringify(receipt, null, 2))}</div>`));
  }
...
app.get('/api/ops/health', async () => ({
  ...
  classroom: classroomCredentialStatus(),
  observedAtUtc: new Date().toISOString()
}));
...
app.get('/api/ops/security', async () => ({
  ...
  controls: {
    ...
    classroomWriteGateConfigured: Boolean(process.env.RTPU_CLASSROOM_INTEGRATION_KEY)
  }
}));
...
app.get('/api/ops/quotas', async () => ({
  ...
  policy: {
    ...
    externalProviderQuotas: 'provider-managed',
...
app.get('/api/ops/topology', async () => ({
  ok: true,
  nodes: ['GitHub', 'Render', 'Fastify API', 'Vite React Client', 'Andreaa Engine', 'LMS Integration Plane', 'Google Classroom'],
  edges: [
    ['GitHub', 'Render'],
    ['Render', 'Fastify API'],
    ['Fastify API', 'Vite React Client'],
    ['Fastify API', 'Andreaa Engine'],
    ['Fastify API', 'LMS Integration Plane'],
    ['LMS Integration Plane', 'Google Classroom']
  ]
}));
...
app.get('/api/ops/support', async () => ({
  ...
  incidentSequence: ['deployment status', 'application health', 'runtime logs', 'provider-specific state', 'rollback decision']
}));
